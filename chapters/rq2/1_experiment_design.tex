...
To keep the relations and context we decided to train a model per project.
\newline
...
\newline
Bezdek et al., also researched cluster validity for the fuzzy c-means model\cite{Bezdek1995}.
...
Fortunately there are some techniques available to estimate the cluster amount.
During model selection we discussed the mountain and subtractive models. 
These models are designed to estimate the amount of clusters in a data set.
...
We will select and implement the subtractive clustering model to estimate the amount of clusters.
We can use this amount for the cluster amount parameter in the fuzzy c-means algorithm.
...
To validate our machine learning model we will calculate the weighed mutation score and cluster accuracy based on the clusters calculated by the model.
